    ...
    description{VK_STRUCTURE_TYPE_TENSOR_DESCRIPTION_ARM}
{
	...
	description.pDimensions        = dimensions.data();        // Note we point to the dimensions array stored in this object, not the one passed in (which will be destroyed!)
    ...
    vkb::core::VulkanResourceC<VkTensorARM>(VK_NULL_HANDLE, &device)        // Handle will be set later in the constructor
{
	...
	// Create tensor
	VkTensorARM tensor = VK_NULL_HANDLE;
	VkResult    res    = vkCreateTensorARM(device.get_handle(), &create_info, nullptr, &tensor);
...
DataGraphPipelineLayout::DataGraphPipelineLayout(vkb::core::DeviceC &device, std::set<uint32_t> tensor_bindings) :
